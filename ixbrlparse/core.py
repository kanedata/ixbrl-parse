...
class IXBRL:
        ...
        for s in resources.find_all(["xbrli:context", "context"]):
            self.contexts[s["id"]] = ixbrlContext(
                **{
                    "_id": s["id"],
                    "entity": {
                        "scheme": s.find(["xbrli:identifier", "identifier"])["scheme"]
                        if s.find(["xbrli:identifier", "identifier"])
                        else None,
                        "identifier": s.find(["xbrli:identifier", "identifier"]).text
                        if s.find(["xbrli:identifier", "identifier"])
                        else None,
                    },
                    "segments": [
                    ...
                    "instant": s.find(["xbrli:instant", "instant"]).text
                    if s.find(["xbrli:instant", "instant"])
                    else None,
                    "startdate": s.find(["xbrli:startDate", "startDate"]).text
                    if s.find(["xbrli:startDate", "startDate"])
                    else None,
                    "enddate": s.find(["xbrli:endDate", "endDate"]).text
    ...
    def _get_units(self):
        ...
        for s in resources.find_all(["xbrli:unit", "unit"]):
            self.units[s["id"]] = (
                s.find(["xbrli:measure", "measure"]).text
